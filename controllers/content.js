...
exports.getAllMovies = async (req, res, next) => {
    try {
        ...
        console.log(req.query);
        const query = "select * from content order by id desc";
        database.executeQuery(res, "", query);
    } catch (e) {
        const response = {
            'status_code': 500,
...
exports.getAllContentGenre = async (req, res, next) => {
    const { genreid } = req.params;
    try {
        const query = `SELECT * FROM content WHERE fk_genre_id = ${genreid}`+` ORDER BY id DESC`;
        database.executeQuery(res, "", query);
    } catch (e) {
        const response = {
            'status_code': 500,
...
exports.getAllContent = (req, res) => {
    try {
        const query = `SELECT * FROM content WHERE approved = 1 ORDER BY id DESC`;
        database.executeQuery(res, "", query);
    } catch (e) {
        const response = {
            'status_code': 500,
...
exports.getfavoriteContent = (req, res) => {
    try {
        const query = `SELECT * FROM  favouritecontent`;
        database.executeQuery(res, "", query);
    } catch (e) {
        const response = {
            'status_code': 500,
...
exports.uploadContent = (req, res) => {
    try {
        ...
        const t = req.file.mimetype.split("/")[0];
        var type='';
        if (t=="video"){
            type='0';
        }
        ...
        query = `INSERT INTO content ( title, contentURL, duration, rating, fk_genre_id, Type, created_at, approved) VALUES ( '${title}', '${contentURL}','${duration}',  '${rating}', ${fk_genre_id}, '${type}', '${created_at}',  ${approved} )`;
        database.con.query(query, (err, result) => {
            if (err) {
                console.log(err);
                res.status(500).json({
...
exports.favoruploadContent = (req, res) => {
    try {
        let query = "";
        const bearerHeader = req.headers['authorization'];
        const url = req.protocol + "://" + req.get("host");
        ...
        query = `INSERT INTO favouritecontent ( title, contentURL, thumbnailURL, duration, fk_genre_id, rating, approved) VALUES ( '${title}', '${contentURL}', '${thumbnailURL}', '${duration}', '${fk_genre_id}', '${rating}',  ${approved} )`;
        database.con.query(query, (err, result) => {
            if (err) {
                console.log(err);
                res.status(500).json({
